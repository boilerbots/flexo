...
#include "k1_control/k1_control.h"
...
controller_interface::CallbackReturn RobotController::on_init()
{
  ...
  for (auto name: joint_names_) {
    RCLCPP_INFO(this->get_node()->get_logger(), "Join %s", name.c_str());
  }
#endif
...
PLUGINLIB_EXPORT_CLASS(k1_control::RobotController, controller_interface::ControllerInterface)
